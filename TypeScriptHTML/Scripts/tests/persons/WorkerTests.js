...
describe("Person", function () {
    ...
    describe("introduce()", function () {
        it("Person should have correct introduce result", function () {
        ...
        it("Worker should cast to Person and have correct introduce result from parent class", function () {
            var someWorker = new Persons.Worker('Ivan', 'Petkov', Gender.Male, 32, { experince: 8, email: 'ivan@epam.com' });
            var basePartner = someWorker;
            if (basePartner.introduce() != "Hello, I'm Ivan Petkov - 8 years experiance email:ivan@epam.com") {
                throw new Error("Not return correct result from method introduce!");
            }
        });
        it("Worker should add correct Jobs with Responsibilities", function () {
            var someWorker = new Persons.Worker('Ivan', 'Petkov', Gender.Male, 32, { experince: 8, email: 'ivan@epam.com' });
